...
ggsave("smelt_2019_allseasons/figures/allzoop.png", dpi=250, height=6, width=10, units="in")
...
ggsave("smelt_2019_allseasons/figures/macrozoop.png", dpi=250, height=6, width=10, units="in")
```
...
#order locations
caught.sea$Location <- factor(caught.sea$Location, levels=c("DWSC", "RV", "Yolo", "SM"))

ggplot() +
  geom_bar(position = "fill", stat = "identity", data=caught.sea, aes (x=Location, y=sum.count, fill=Taxa), colour="black", width = .5) +
...
ggplot() +
  geom_bar(position = "fill", stat = "identity", data=caught.sea2, aes (x=Location, y=sum.count, fill=Taxa), colour="black", width = .5) +
  facet_wrap(~Season, ncol = 1) +
  theme_bw() + 
  viridis::scale_fill_viridis(discrete = TRUE, option = "plasma") +
  labs(x = "Location") +
