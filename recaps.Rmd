...
recaps    =     mutate(recaps, fishID = paste("smelt", 1:nrow(recaps)),
                Type = case_when(Type == "header" ~ "Soft",
                                 TRUE ~ Type)) %>%
  rename(Adclip = `Ad clip`, Mark = `VIE Color/Position`) %>%
  select(-Type, -Site)
str(recaps)
...
ggplot(nndist, aes(x = Release, y = Distance, fill = Release)) + geom_boxplot()+
  ylab("Distance Between Recaptures")

```

Now let's bring in the time aspect

```{r}

recapstime = read_excel("RecapturesSummary_1.xlsx", sheet = "REcapture Distances")
