...
#Huh, so percentage recaptures is highly to turbidityy.

turbm2 = lmer(RecapRate ~ log(weekturb) + (1|BroodYear), data = releasestat)
summary(turbm2)
plot(allEffects(turbm2))

ggplot(releasestat, aes(x = log(weekturb), y = RecapRate))+
  geom_point(aes(shape = as.factor(BroodYear)))+ geom_smooth(method = "lm")
